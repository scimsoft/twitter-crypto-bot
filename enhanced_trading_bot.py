#!/usr/bin/env python3
"""
Enhanced Crypto Trading Bot with Price Tracking, Profit Calculation and Strategy Optimization
"""
...
class EnhancedTradingBot:
    def __init__(self, initial_capital=10000, coin_symbol="DOGE"):
        """
        Initialize the enhanced trading bot with initial capital
        """
        ...
        try:
            # Initialize tweepy client with all credentials for full functionality
            ...
            try:
                ...
                self.check_interval_minutes = 60  # default to 60 minutes if config unavailable
            ...
            logger.info(f"Enhanced Trading Bot initialized with ${initial_capital} in {coin_symbol}")
            
        except Exception as e:
            logger.error(f"Failed to initialize Enhanced Trading Bot: {str(e)}")
            raise
    ...
    def analyze_sentiment(self):
        """
        Analyze sentiment from crypto influencer tweets
        """
        ...
        for username in crypto_accounts:
            try:
                ...
                tweets = self.twitter_client.get_users_tweets(
                    id=user_id,
                    max_results=3,  # Get last 3 tweets
                ...
                for tweet in tweets.data:
                    ...
                    positive_keywords = ['bull', 'moon', 'buy', 'hodl', 'strong', 'good', 'great', 'up', 'profit', 'gain']
                    negative_keywords = ['bear', 'sell', 'dump', 'crash', 'bad', 'down', 'loss', 'fear', 'weak', 'concern']
                    ...
                    if pos_score + neg_score > 0:
                        tweet_score = (pos_score - neg_score) / (pos_score + neg_score)
                        if tweet_score > 1:
                            tweet_score = 1
                        if tweet_score < -1:
                            tweet_score = -1
                ...
                logger.debug(f"Error analyzing {username}: {str(e)}")
                continue
        
        # Calculate average sentiment
        avg_sentiment = total_score / tweet_count if tweet_count > 0 else 0
    ...
    def get_interface_data(self):
        ...
        recent_sentiment = self.sentiment_history[-1]['score'] if self.sentiment_history else 0
    ...
    def print_interface(self):
        ...
        print("\n" + "="*60)
        print(f"           ENHANCED CRYPTO TRADING BOT INTERFACE")
        print("="*60)
    ...
    def run_continuous(self, interval_minutes=None):
        ...
        while True:
            try:
                ...
                sleep_minutes = self.check_interval_minutes if hasattr(self, "check_interval_minutes") else 15  # default to 15 minutes if not set
